...
export function HeroSection() {
  return (
    <section className="relative overflow-hidden bg-bg [background-image:linear-gradient(var(--grid-line)_1px,transparent_1px),linear-gradient(90deg,var(--grid-line)_1px,transparent_1px)] [background-size:56px_56px]">
      {/* Starfield + ambient dual-tone glow — sky accent top-right, violet accent-2 bottom-left */}
      ...
      <div className="absolute top-1/3 -left-40 w-[420px] h-[420px] rounded-full blur-[110px] pointer-events-none bg-accent-2 opacity-[0.06]" />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-24 sm:py-28 flex flex-wrap gap-14 items-center">
        <motion.div
          ...
          className="glass-panel rounded-2xl p-7 sm:p-9 flex-1 min-w-[300px] max-w-xl"
        >
          ...
          <h1 className="font-display font-bold text-4xl sm:text-5xl leading-[1.08] tracking-tight text-ink">
            ...
            <br />
            and build the <span className="text-gradient-accent">AI</span> that runs them.
          </h1>

          <p className="mt-6 text-base sm:text-lg text-ink-secondary leading-relaxed">
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {["React", "Next.js", "Node.js", "TypeScript", "Python", "AWS", "Docker", "Claude"].map((tech) => (
              <span
                key={tech}
                className="font-mono text-[11px] font-medium text-ink-secondary bg-bg-alt border border-border rounded-full px-3 py-1"
              >
                {tech}
              </span>
            ))}
          ...
          <p className="mt-6 font-mono text-xs text-ink-tertiary tracking-wide leading-loose">
        ...
        >
          <div className="glass-panel rounded-2xl overflow-hidden border-accent-border shadow-[0_30px_60px_-20px_rgba(0,0,0,0.35)]">
